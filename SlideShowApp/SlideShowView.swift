...
struct SlideShowView: View {
    ...
    @ObservedObject private var cache: AssetCache
        ...
        case .crossFade, .kenBurns:
            ZStack {
                if let item = cache.currentItem, displayedIndex == vm.currentIndex {
                    MediaItemView(item: item, vm: vm).ignoresSafeArea()
                }
                if nextIndex != nil, let item = cache.currentItem {
                    MediaItemView(item: item, vm: vm).ignoresSafeArea().opacity(crossFadeOpacity)
                }
            }
        case .slide:
            GeometryReader { geo in
                ZStack {
                    if let item = cache.currentItem, displayedIndex == vm.currentIndex {
                        MediaItemView(item: item, vm: vm).ignoresSafeArea()
                            .offset(x: nextIndex != nil ? -geo.size.width * (1 - crossFadeOpacity) : 0)
                    }
                    if nextIndex != nil, let item = cache.currentItem {
                        MediaItemView(item: item, vm: vm).ignoresSafeArea()
                            .offset(x: geo.size.width * (1 - crossFadeOpacity))
                    }
                }
            }
        }
    ...
    private func animateTransition(to newIndex: Int) {
        ...
        withAnimation(.easeInOut(duration: duration)) { crossFadeOpacity = 1.0 }
    ...
    private var controlOverlay: some View {
        VStack {
            HStack {
                ...
                Spacer()
                if vm.hasMedia {
                    Text(String(format: String(localized: "slideshow.counter"), vm.currentIndex + 1, vm.totalCount))
            ...
            HStack(spacing: 48) {
                ...
                Button(action: { vm.goNext(); resetHideTimer() }) {
                    Image(systemName: "forward.fill").font(.largeTitle).foregroundColor(.white)
                }
                .disabled(!vm.hasMedia)
            }
            .padding(.bottom, 60)
    ...
    private var emptyPlaceholder: some View {
        VStack(spacing: 24) {
            Image(systemName: "photo.stack")
                .font(.system(size: 72)).foregroundColor(.white.opacity(0.4))
            Text(String(localized: "slideshow.empty.message"))
                .foregroundColor(.white.opacity(0.6)).font(.headline)
            Button(action: { vm.showPicker = true }) {
                Label(String(localized: "slideshow.empty.button"), systemImage: "photo.on.rectangle")
